...
//    [[self layer] setBorderColor:[[UIColor colorWithRed:209/255.0 green:209/255.0 blue:209/255.0 alpha:1.0] CGColor]];
    ...
    _indicatorLayer = [self createIndicatorWithColor:_highlightColor
                                         andPosition:CGPointMake(self.frame.size.width - 15, self.frame.size.height/2.0)];
    ...
    _textLayer = [self createTextLayerWithText:@"iMac" color:[UIColor colorWithHexString:@"#8F9394"] withPosition:position];
    ...
    [_tableView setSeparatorColor:[[UIColor blackColor] colorWithAlphaComponent:0.1]];
...
- (UIView *)testView {
    if (!_testView) {
        ...
        _testView.backgroundColor = whitecolor;
    }
    return _testView;
}
...
- (void)animateTableView:(UITableView *)tableView show:(BOOL)isShow completion:(void(^)(void))completion {
    ...
    if (isShow) {
        ...
        [self.testView setFrame:CGRectMake(self.frame.origin.x, self.frame.origin.y+self.frame.size.height, self.frame.size.width, 0)];
        ...
        if (tableView_Height >= 275) {
            tableView_Height = 275;
        }
        
        [UIView animateWithDuration:0.25 delay:0.0 options:UIViewAnimationOptionCurveEaseOut animations:^{
            
            [self.testView setFrame:CGRectMake(self.frame.origin.x, self.frame.origin.y+self.frame.size.height, self.frame.size.width, tableView_Height)];
            self.testView.layer.shadowColor = [[UIColor blackColor] colorWithAlphaComponent:0.27].CGColor;
            self.testView.layer.shadowOffset = CGSizeMake(0, 2);
...
- (CGSize)p_calculateTitleSizeWithText:(NSString *)text {
    ...
    NSDictionary *dic = @{NSFontAttributeName : [UIFont fontWithName:@"PingFangHK-Regular" size:fontSize]};
