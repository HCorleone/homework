...
@interface OldLoginViewController () <WeiboSDKDelegate, WXApiDelegate>
...
- (void)setUpViews {
    ...
    //app图标
    UIImageView *appImgView = [UIImageView new];
    appImgView.image = [UIImage imageNamed:@"logo_120_120-rect"];
    [contentView addSubview:appImgView];
    [appImgView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.size.mas_equalTo(CGSizeMake(Main_Screen_Width * 136 / 640, Main_Screen_Width * 136 / 640));
        make.centerX.mas_equalTo(contentView.mas_centerX);
        if (isiPhone4) {
            make.top.mas_equalTo(contentView.mas_top).with.offset(Main_Screen_Height * 33 / 1136);
        }else{
            make.top.mas_equalTo(contentView.mas_top).with.offset(Main_Screen_Height * 48.9 / 1136);
        }
        
    }];
    self.appImgView = appImgView;
    
    
    //圆角
    appImgView.layer.cornerRadius = 5.0;
    appImgView.clipsToBounds = NO;
    ...
    if([XUtil isAudit]){
        ...
        titleLabel.text = @"请选择登录方式";
    }
    titleLabel.textAlignment = NSTextAlignmentCenter;
    titleLabel.font = font14;
    [titleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.size.mas_equalTo(CGSizeMake(Main_Screen_Width * 196 / 640, [XUtil sizeWithString:titleLabel.text font:font14 maxSize:CGSizeMake(MAXFLOAT, MAXFLOAT)].height));
        make.centerX.mas_equalTo(contentView.mas_centerX);
        if (isiPhone4) {
            make.top.mas_equalTo(appImgView.mas_bottom).with.offset(Main_Screen_Height * 64 / 1136);
        }else{
            make.top.mas_equalTo(appImgView.mas_bottom).with.offset(Main_Screen_Height * 24 / 1136);
        }
        
    }];
    
    //QQ登录
    CGFloat qqLoginViewX = 0;
    CGFloat qqLoginViewYMargin = Main_Screen_Height * 68 / 1136;
    if (isiPhone4) {
        qqLoginViewYMargin = Main_Screen_Height * 46 / 1136;
    }
    CGFloat qqLoginViewH = 48;
    CGFloat qqLoginViewW = Main_Screen_Width - qqLoginViewX * 2;
    OldLoginView *qqLoginView = [[OldLoginView alloc] init];
    qqLoginView.typeLabel.text = @"QQ登录";
    qqLoginView.iconImgView.image = [UIImage imageNamed:@"qq_icon"];
    ...
    [qqLoginView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.mas_equalTo(titleLabel.mas_bottom).with.offset(qqLoginViewYMargin);
    }];
    qqLoginView.backgroundColor = [UIColor whiteColor];
    
    //微信登录
    CGFloat loginViewYMargin = Main_Screen_Height * 45 / 1136;
    if (isiPhone4) {
        loginViewYMargin = Main_Screen_Height * 34 / 1136;
    }
    OldLoginView *wechatLoginView = [[OldLoginView alloc] init];
    wechatLoginView.typeLabel.text = @"微信登录";
    wechatLoginView.iconImgView.image = [UIImage imageNamed:@"weixin_icon"];
    ...
    [wechatLoginView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        if(![WXApi isWXAppInstalled] || [[UrlBuild getAppName] isEqualToString:@"英语听力"]|| [[UrlBuild getAppName] isEqualToString:@"英语阅读"]){
            make.size.mas_equalTo(CGSizeMake(qqLoginViewW, 0));
            make.top.mas_equalTo(qqLoginView.mas_bottom);
        }else{
            make.size.mas_equalTo(CGSizeMake(qqLoginViewW, qqLoginViewH));
            make.top.mas_equalTo(qqLoginView.mas_bottom).with.offset(loginViewYMargin);
        }
        }];
    wechatLoginView.backgroundColor = [UIColor whiteColor];
    if(![WXApi isWXAppInstalled]){
        wechatLoginView.hidden = YES;
    }
    //新浪微博登录
    OldLoginView *sinaWeiBoLoginView = [[OldLoginView alloc] init];
    sinaWeiBoLoginView.typeLabel.text = @"微博登录";
    sinaWeiBoLoginView.iconImgView.image = [UIImage imageNamed:@"weibo_icon"];
    ...
    [sinaWeiBoLoginView mas_makeConstraints:^(MASConstraintMaker *make) {
        if([WXApi isWXAppInstalled]|| [[UrlBuild getAppName] isEqualToString:@"英语听力"]|| [[UrlBuild getAppName] isEqualToString:@"英语阅读"]){
            make.top.mas_equalTo(wechatLoginView.mas_bottom).with.offset(loginViewYMargin);
        }else{
            make.top.mas_equalTo(qqLoginView.mas_bottom).with.offset(loginViewYMargin);
        }
        make.left.equalTo(contentView).with.offset(qqLoginViewX);
    ...
    sinaWeiBoLoginView.backgroundColor = [UIColor whiteColor];
    ...
    //网易邮箱登录
    OldLoginView *neteaseLoginView = [[OldLoginView alloc] init];
    neteaseLoginView.typeLabel.text = @"邮箱登录";
    neteaseLoginView.iconImgView.image = [UIImage imageNamed:@"ic_netease"];
    UITapGestureRecognizer *neteaseLogin = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(toNetEaseMailLogin:)];
    [neteaseLoginView addGestureRecognizer:neteaseLogin];
    [contentView addSubview:neteaseLoginView];
    [neteaseLoginView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(contentView).with.offset(qqLoginViewX);
        make.size.mas_equalTo(CGSizeMake(qqLoginViewW, qqLoginViewH));
        if([[UrlBuild getAppName] isEqualToString:@"英语百科"]|| [[UrlBuild getAppName] isEqualToString:@"英语视频"]|| [[UrlBuild getAppName] isEqualToString:@"英语听力"]|| [[UrlBuild getAppName] isEqualToString:@"英语阅读"]){
            make.top.mas_equalTo(wechatLoginView.mas_bottom).with.offset(loginViewYMargin);
        }else{
            make.top.mas_equalTo(sinaWeiBoLoginView.mas_bottom).with.offset(loginViewYMargin);
        }
    }];
    neteaseLoginView.backgroundColor = [UIColor whiteColor];
    
    //注册网易邮箱
    UILabel *toRegisterNeteaseMailLabel = [UILabel new];
    toRegisterNeteaseMailLabel.font = font14;
    toRegisterNeteaseMailLabel.textColor = [XUtil hexToRGB:MAIN_COLOR];
    toRegisterNeteaseMailLabel.text = @"注册网易邮箱";
    toRegisterNeteaseMailLabel.userInteractionEnabled = YES;
    CGSize textSize = [XUtil sizeWithString:toRegisterNeteaseMailLabel.text font:font14 maxSize:CGSizeMake(MAXFLOAT, MAXFLOAT)];
    UITapGestureRecognizer *toRegisterNetease = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(toRegisterNetease:)];
    [toRegisterNeteaseMailLabel addGestureRecognizer:toRegisterNetease];
    [contentView addSubview:toRegisterNeteaseMailLabel];
    [toRegisterNeteaseMailLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.size.mas_equalTo(textSize);
        make.centerX.mas_equalTo(contentView.mas_centerX);
        if (isiPhone4) {
            make.top.mas_equalTo(neteaseLoginView.mas_bottom).with.offset(32 * Main_Screen_Height / 1136);
        }else{
            make.top.mas_equalTo(neteaseLoginView.mas_bottom).with.offset(112 * Main_Screen_Height / 1136);
        }
    }];
    
    neteaseLoginView.hidden = NO;
    toRegisterNeteaseMailLabel.hidden = NO;
    [contentView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.bottom.equalTo(toRegisterNeteaseMailLabel.mas_bottom).with.offset(2 * Main_Screen_Height / 1136);
    }];
}

//注册网易邮箱
- (void)toRegisterNetease:(UITapGestureRecognizer *)ges{
    WebViewController *vc = [WebViewController new];
    vc.url = [NSURL URLWithString:kNeteaseMailREGISTER_URL];
    [self.navigationController pushViewController:vc animated:YES];
    //    [[UIApplication sharedApplication] openURL:[NSURL URLWithString:kNeteaseMailREGISTER_URL]];
}
...
//网易邮箱登录
- (void)toNetEaseMailLogin:(UITapGestureRecognizer *)ges{
    NetEaseMailController *vc = [[NetEaseMailController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}
